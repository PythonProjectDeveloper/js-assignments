...
function findFirstSingleChar(str) {
    ...
    for (let symbol of str) {
        if (symbols.has(symbol)) symbols.set(symbol, symbols.get(symbol) + 1);
        else symbols.set(symbol, 1);
    }
    
    const result = Array.from(symbols.entries()).find((val) => val[1] === 1);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    ...
    const endBraket = isEndIncluded ? `]` : `)`;
    const value = (a > b) ? `${b}, ${a}` : `${a}, ${b}`;
    return `${startBracket}${value}${endBraket}`;
}
...
function isBracketsBalanced(str) {
    while(true) {
        ...
        if(newStr === str) return newStr.length === 0;
        str = newStr;
    }
}
...
function getCommonDirectoryPath(pathes) {
    const sep = '/';
    pathes = pathes.map((val) => val.split(sep));
    const template = pathes[0];
    const path = [];

    for (let idx in template) {
        if (pathes.every((val) => val[idx] === template[idx])) path.push(template[idx] + sep);
        else break;
    };
